...
| _"the font family Satoshi Variable does not exist"_ | Satoshi and Erode are installed on Max's machine, not shared with the file. Neither is vendored in `public/fonts/` yet, so the built site renders in fallbacks. Expected, not a bug to fix mid-task. |
